...
The **`rbac` auth plugin** is enabled by default (`"auth": "rbac"` in `api/plugins.json`). On a fresh install, set `UMPIRE_ADMIN_USERNAME` and `UMPIRE_ADMIN_PASSWORD` before first start, then sign in and create additional users in the rbac **Settings** panel if needed. To run without login (open mode), disable auth under **Settings → Plugin manager → Auth** (immediate). See [Authentication](core.md#authentication-and-rbac) in the core guide.
