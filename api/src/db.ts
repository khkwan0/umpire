...
export function createTarget(url: string, intervalSeconds: number, enabled = true): Target {
  const result = getDb()
    .prepare(
...
export function updateTarget(
  ...
  const interval =
    patch.interval_seconds ?? existing.interval_seconds
  const enabled =
...
export function getTargetState(targetId: number): TargetState | undefined {
  return getDb()
    .prepare(`SELECT * FROM target_state WHERE target_id = ?`)
    .get(targetId) as TargetState | undefined
}

export function recordCheckResult(input: {
  targetId: number
  ok: boolean
  ...
  // Keep last 500 results per target
  database
    .prepare(
...
export function markAlertSent(targetId: number): void {
  getDb()
    .prepare(
...
export function listRecentResults(targetId: number, limit = 50): CheckResult[] {
  return getDb()
    .prepare(
